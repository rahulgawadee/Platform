...
import { FiBarChart2, FiFileText, FiUser, FiBook, FiMenu, FiX, FiChevronLeft } from 'react-icons/fi';
...
const Sidebar = ({ activeTab, setActiveTab, isMobile, toggleSidebar }) => {
  ...
  return (
    <div className={`bg-white shadow-lg flex flex-col transition-all duration-300 ease-in-out 
      ...
      <div className={`flex items-center p-4 ${collapsed ? 'justify-center' : 'justify-between'}`}>
        {!collapsed && (
          <div className="flex items-center gap-3">
            ...
            <span className="text-lg font-semibold text-gray-800">Praktikly</span>
          </div>
        )}
        ...
        >
          {isMobile ? <FiX size={24} /> : collapsed ? <FiMenu size={24} /> : <FiChevronLeft size={24} />}
        </button>
      </div>
      ...
      <nav className="flex-1 flex flex-col gap-1 p-2 overflow-y-auto">
        {navItems.map(({ id, icon, label }) => (
          ...
          >
            <span className={`${activeTab === id ? 'text-blue-600' : 'text-gray-500'}`}>
              {icon}
            </span>
